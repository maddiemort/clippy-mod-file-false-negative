...
REPLACE_IN=(flake.nix Cargo.toml Cargo.lock README.md src/lib.rs src/main.rs)
...
if command -v direnv >/dev/null 2>&1; then
    read -rp "Load flake.nix using direnv ('direnv allow')? (y/n): " ALLOW

    if [[ $ALLOW == "y" ]]; then
        echo "Running 'direnv allow'..."
        direnv allow
    else
        echo "Not running direnv"
    fi
fi
